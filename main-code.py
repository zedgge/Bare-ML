def dot(a, b):
    return sum(a[i] * b[i] for i in range(len(a)))

def matmul(vec, mat):
    return [dot(vec, col) for col in zip(*mat)]

def sigmoid(x):
    return 1 / (1 + pow(2.718281828459045, -x))

def sigmoid_derivative(x):
    s = sigmoid(x)
    return s * (1 - s)

def softmax(x):
    max_x = max(x)
    exps = [pow(2.718281828459045, i - max_x) for i in x]
    sum_exps = sum(exps)
    return [j / sum_exps for j in exps]

def one_hot(label, size=10):
    vec = [0.0] * size
    vec[label] = 1.0
    return vec

def argmax(vec):
    max_i = 0
    for i in range(1, len(vec)):
        if vec[i] > vec[max_i]:
            max_i = i
    return max_i

def load_csv(path, max_rows):
    data = []
    with open(path, 'r') as f:
        lines = f.readlines()
    for i in range(1, min(len(lines), max_rows + 1)):
        parts = lines[i].strip().split(',')
        label = int(parts[0])
        pixels = [float(p) / 255.0 for p in parts[1:]]
        data.append((pixels, one_hot(label)))
    return data

class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size):
        self.w1 = [[(i * j % 5 - 2) / 10.0 for j in range(hidden_size)] for i in range(input_size)]
        self.b1 = [0.0] * hidden_size
        self.w2 = [[(i * j % 7 - 3) / 10.0 for j in range(output_size)] for i in range(hidden_size)]
        self.b2 = [0.0] * output_size

    def forward(self, x):
        self.z1 = [dot(x, col) + self.b1[i] for i, col in enumerate(zip(*self.w1))]
        self.a1 = [sigmoid(z) for z in self.z1]
        self.z2 = [dot(self.a1, col) + self.b2[i] for i, col in enumerate(zip(*self.w2))]
        self.a2 = softmax(self.z2)
        return self.a2

    def backward(self, x, y, lr):
        output_error = [self.a2[i] - y[i] for i in range(len(y))]
        dz2 = output_error

        dw2 = [[self.a1[i] * dz2[j] for j in range(len(dz2))] for i in range(len(self.a1))]
        db2 = dz2

        da1 = [sum(self.w2[i][j] * dz2[j] for j in range(len(dz2))) for i in range(len(self.a1))]
        dz1 = [da1[i] * sigmoid_derivative(self.z1[i]) for i in range(len(da1))]

        dw1 = [[x[i] * dz1[j] for j in range(len(dz1))] for i in range(len(x))]
        db1 = dz1

        for i in range(len(self.w2)):
            for j in range(len(self.w2[0])):
                self.w2[i][j] -= lr * dw2[i][j]
        for i in range(len(self.b2)):
            self.b2[i] -= lr * db2[i]

        for i in range(len(self.w1)):
            for j in range(len(self.w1[0])):
                self.w1[i][j] -= lr * dw1[i][j]
        for i in range(len(self.b1)):
            self.b1[i] -= lr * db1[i]

def train(network, data, epochs, lr):
    for epoch in range(epochs):
        total_loss = 0
        correct = 0
        for x, y in data:
            output = network.forward(x)
            network.backward(x, y, lr)
            pred = argmax(output)
            actual = argmax(y)
            correct += int(pred == actual)
        acc = correct / len(data) * 100
        print("Epoch", epoch + 1, "Accuracy:", acc, "%")

def test(network, data):
    correct = 0
    for x, y in data:
        output = network.forward(x)
        if argmax(output) == argmax(y):
            correct += 1
    return correct / len(data) * 100

def main():
    TRAIN_PATH = "mnist_train.csv"
    TEST_PATH = "mnist_test.csv"
    MAX_TRAIN = 1000
    MAX_TEST = 200

    print("[+] Loading data...")
    train_data = load_csv(TRAIN_PATH, MAX_TRAIN)
    test_data = load_csv(TEST_PATH, MAX_TEST)

    print("[+] Training...")
    net = NeuralNetwork(784, 64, 10)
    train(net, train_data, epochs=5, lr=0.1)

    print("[+] Testing...")
    acc = test(net, test_data)
    print("Test Accuracy:", acc, "%")

main()
